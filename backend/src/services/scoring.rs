...
impl MFCCExtractor {
    ...
    fn apply_hamming_window(&self, frame: &[f32]) -> Vec<f32> {
        frame.iter()
            .enumerate()
            .map(|(i, &sample)| {
                let window_value = 0.54 - 0.46 * (2.0 * std::f32::consts::PI * i as f32 / (FRAME_SIZE - 1) as f32).cos();
                sample * window_value
            })
            .collect()
    }
    ...
    fn compute_mfcc(&self, mel_energies: &[f32]) -> Vec<f32> {
        ...
        for i in 0..MFCC_COEFFS {
            ...
            for (j, &log_energy) in log_mel.iter().enumerate() {
                let cos_term = (std::f32::consts::PI * i as f32 * (2 * j + 1) as f32 / (2.0 * log_mel.len() as f32)).cos();
        ...
        for i in 0..num_filters {
            ...
            for j in 0..filter.len() {
                ...
                if distance <= width as f32 {
                    filter[j] = 1.0 - distance / width as f32;
                }
            }
...
impl PronunciationScorer {
    ...
    pub fn score_pronunciation(
        ...
        Ok(PronunciationScore {
            overall_pct: percentage_score.max(0.0).min(100.0),
            per_phoneme,
            alignment_cost,
    ...
    fn calculate_per_phoneme_scores(
        ...
        for i in 0..num_phonemes {
            ...
            if start_idx < user_features.len() && start_idx < reference_features.len() {
                ...
                scores.insert(phoneme_key, segment_score.max(0.0).min(100.0));
            } else {
...
mod tests {
    ...
    fn test_pronunciation_scoring() {
        ...
        let score = scorer.score_pronunciation(&user_audio, &reference_audio).unwrap();
