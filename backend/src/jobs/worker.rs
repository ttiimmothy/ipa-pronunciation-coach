...
impl JobWorker {
    ...
    async fn execute_job(&mut self, job: &Job) -> Result<JobResult> {
        match &job.job_type {
            JobType::PronunciationScoring {
                ...
                self.process_pronunciation_scoring(*recording_id, *word_id, dialect, audio_url).await
            }
            JobType::SearchIndexUpdate { word_id } => {
                self.process_search_index_update(*word_id).await
            }
        }
    }
    ...
    async fn process_pronunciation_scoring(
        ...
        info!("Processing pronunciation scoring for recording {}", recording_id);
        ...
        // Score pronunciation
        let score = self.scorer.score_pronunciation(&audio_data, &reference_audio)?;
    ...
    async fn notify_score_completion(
        ...
        info!("Score completed for recording {}: {}%", recording_id, score.overall_pct);
        Ok(())
    }
    ...
    async fn handle_job_result(&self, job: &Job, result: &JobResult) -> Result<()> {
        ...
        conn.set_ex::<_, _, ()>(&result_key, result_json, 3600).await?; // 1 hour TTL
    ...
    async fn handle_job_failure(&self, job: &Job, error: &str) -> Result<()> {
        ...
        if job.retry_count < job.max_retries {
            ...
            warn!("Job {} failed, retrying ({}/{})", job.id, retry_job.retry_count, job.max_retries);
        } else {
            ...
            let result_key = format!("job_result:{}", job.id);
            let result_json = serde_json::to_string(&result)?;
            conn.set_ex::<_, _, ()>(&result_key, result_json, 3600).await?;
            
            error!("Job {} permanently failed after {} retries", job.id, job.max_retries);
        }
